...
# -*- coding: utf-8 -*-
"""
qa_generation/semantic.py 単体テスト

[PORT] anthropic_grace_agent → openai_grace_agent
  - 埋め込み次元数: 3072（変更なし。text-embedding-3-large）
  - デフォルト埋め込みモデル: gemini-embedding-001 → text-embedding-3-large

[Usage]: pytest -vs tests/qa_generation/test_semantic.py
"""
...
def mock_clients():
    with patch('qa_generation.semantic.create_llm_client') as mock_llm_factory, \
        ...
        # Mock Embedding Client（OpenAI text-embedding-3-large: 3072次元）
        mock_emb = MagicMock()
...
class TestSemanticCoverage:
    ...
    def test_generate_embeddings(self, mock_clients):
        ...
        # [PORT] openai text-embedding-3-large: 3072次元
