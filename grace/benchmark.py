...
BENCHMARK_QUERIES: List[Dict[str, str]] = [
    ...
    # Ambiguous / 曘昧（ESCALATE を誘発するテスト）
    {"id": "Q09", "level": "Easy",   "category": "曘昧",
     "text": "最近の重要なニュースを教えて"},
    {"id": "Q10", "level": "Easy",   "category": "曘昧",
     "text": "あの件について詳しく教えて"},
    # Hard / エラー回復・複合
...
class BenchmarkSession:
    ...
    category: str = ""      # 事実検索 / 推論・比較 / 手順説明 / 曘昧
    model: str = ""
    provider: str = ""
    run_number: int = 1
...
class BenchmarkRunner:
    ...
    Planner / Executor のコンストラクタ引数はリポジトリごとに差異があるため、
    model_name あり / なしの両パターンに自動でフォールバックする。
    """
    ...
    def _call_execute(executor: Any, plan: Any) -> Any:
        import types
        # Prefer blocking execute_plan (always computes overall_confidence).
        # Fall back to execute_plan_generator (streaming), then execute.
        ...
        # Generator: StopIteration.value (return value) を優先して捕捉する
