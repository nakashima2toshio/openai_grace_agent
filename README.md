...
日本語 RAG（Retrieval-Augmented Generation）× 自律型エージェントのプロジェクトです。
LLM は **OpenAI GPT**（既定 `gpt-5-mini`、`create_llm_client("openai")` 経由）、Embedding は **OpenAI Embedding**（`text-embedding-3-large`、3072次元、`create_embedding_client("openai")` 経由）を使用し、ベクターDB には Qdrant を利用します。APIキーは `OPENAI_API_KEY` を `.env` に設定します。

#### (1) 自律型Agent — GRACE（OpenAI API利用、スクラッチで作成）

GRACE = **Guided Reasoning with Adaptive Confidence Execution**（適応型計画実行エージェント）

- (1-1) 計画策定（Plan）— 二層計画生成（単純質問はルールベース即時生成 / 複雑質問のみ LLM 生成）
- → 実行（Execute）— 並列プリフェッチ・タイムアウト・動的フォールバック連鎖（rag_search → web_search → ask_user）
- → 信頼度評価（Confidence）— groundedness（根拠妥当性）ブレンド + 温度スケーリング較正
- → 介入判定（Intervention）— SILENT / NOTIFY / CONFIRM / ESCALATE の4段階
- → リプラン（Replan）— PARTIAL / FULL 戦略による計画修正
- ＋ 実行メモリ層（P4）— 実行実績からコレクション事前分布を学習し Planner に反映
...
- (2-2) RAG: Chunkデータの作成（`python -m chunking.csv_text_to_chunks_text_csv`）
- (2-3) RAG: Qdrant(ベクターDB)への登録、検索（`qa_qdrant/make_qa_register_qdrant.py`）
...
```bash
# 1. Qdrant + Redis を起動（compose ファイルは docker-compose/ 配下）
...
> | # | ドキュメント | 説明 |
> | - | ----------- | ---- |
> | R1 | [LLM API 3プロバイダー対比表 v2 (docs/llm_api_comparison_v2.md)](./docs/llm_api_comparison_v2.md) | 主要 LLM プロバイダーの API・クライアント・Embedding 完全比較（最新版） |
> | R2 | [OpenAI 移植仕様書 (docs/openai_port_spec_july01.md)](./docs/openai_port_spec_july01.md) | OpenAI API への移植仕様（移行の設計資料） |
> | R3 | [OpenAI 移植 TODO (docs/openai_port_todo_july01.md)](./docs/openai_port_todo_july01.md) | OpenAI API への移植 TODO・進捗チェックリスト |
> | R4 | [openai/anthropic 実ファイル比較表 (docs/openai_anthropic_file_comparison_june14.md)](./docs/openai_anthropic_file_comparison_june14.md) | リポジトリ間の実ファイル差分比較（移行参考資料） |
> | R5 | [Gemini → Anthropic 移植報告書 (docs/API_migration_gemini2anthropic.md)](./docs/API_migration_gemini2anthropic.md) | 過去の移植完了報告（歴史的参考資料） |
> | R6 | [LLM API 比較表 v1 (docs/llm_api_comparison.md)](./docs/llm_api_comparison.md) | LLM API 比較表（v1・旧版参考） |

---

## （自律型Agent）grace_chat_page.py

画面： agent_rag.py（起動: `uv run streamlit run agent_rag.py --server.port 8501`）
![img.png](assets/lp_gamen_img.png)

個々のドキュメントは、機能ディレクトリー/doc/*.md を参照あれ。
以下は GRACE 自律エージェント（`ui/pages/grace_chat_page.py` ＋ `grace/` パッケージ）の詳細ドキュメントです。
...
| **3.0**    | **2026-02-17** | **ドキュメント体系の整備**。主な変更: (1) 冒頭ドキュメント一覧表を拡張（readme_rag.md / readme_react_reflection.md / readme_autonomous_agent.md を追加）、(2) 番号付きリンク表で参照順を明示 |
| **4.0**    | **2026-07-10** | **OpenAI 移行＋grace 新機能反映の全面最新化**。主な変更: (1) 技術スタック表記を OpenAI GPT（既定 `gpt-5-mini`）/ `text-embedding-3-large`（3072次元）/ `create_llm_client("openai")` / `OPENAI_API_KEY` に統一し、Gemini 前提の記述（gemini-* モデル表・google-genai 依存・GEMINI_API_KEY・provider="gemini"）を全廃、(2) モデル一覧表を `config.py` の `GeminiConfig.AVAILABLE_MODELS` 実値（gpt-5-mini ほか）に差し替え（クラス名は後方互換の名残で実体は OpenAI 設定である旨を注記）、(3) Planner の**二層計画生成**（`PlannerConfig.llm_plan_complexity_threshold=0.7`）・`is_ambiguous_query` による曖昧クエリ→明確化計画・`_generate_plan_with_retry`・`_prioritized_collection`（実行メモリ層）を追記、(4) Executor の並列プリフェッチ（`_prefetch_parallel_searches`、max_parallel_steps=4）・`_run_tool_with_timeout`・動的フォールバック連鎖（rag_search→web_search→ask_user）・CONFIRM自動続行/ESCALATEのみ一時停止・低信頼度リプランの検索系限定（#64）を追記、(5) **S1**: GroundednessVerifier・`_blend_groundedness_confidence`・calibration.py（温度スケーリング較正、config/calibration.json）を新規記載、(6) **P4**: memory.py（実行メモリ層、logs/grace_memory.jsonl、min_count=3/min_score=0.6）を新規記載、(7) benchmark.py（BenchmarkRunner / BENCHMARK_QUERIES）と ExecutionResult のベンチマーク計測フィールド（rag_max_score / rag_search_count / web_search_used）を追記、(8) 設定リファレンスを実コード準拠に全面更新（PlannerConfig / ExecutorConfig / MemoryConfig / WebSearchConfig 等を追加）、(9) 依存表を是正（google-genai → openai）、廃止スクリプト（setup.py / server.py / a-prefixed）への言及を現行コマンド（`docker compose` / `uv run streamlit run agent_rag.py`）へ置換、(10) Mermaid 図を黒背景・白文字規約（classDef / sequenceDiagram init ヘッダー）に統一、(11) 冒頭ドキュメント一覧に OpenAI 移植仕様書・TODO 等を追加 |
...
| ファイル                                                   | 内容                                          |
| ---------------------------------------------------------- | --------------------------------------------- |
| [readme_autonomous_agent.md](readme_autonomous_agent.md)   | GRACEアーキテクチャ（Plan+Executor）の設計・IPO 詳細 |
